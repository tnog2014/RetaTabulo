...
@SpringBootTest
public class BoardFormValidationTest {
	...
	private BoardForm targetForm = new BoardForm();

	private BindingResult bindingResult = new BindException(targetForm, "BoardForm");
	...
	public void assertData(final Object[][] expectations) {
		Set<ConstraintViolation<BoardForm>> violations = validator.validate(targetForm);
//				ValidGroup1.class, ValidGroup2.class, ValidGroup3.class);
		assertEquals(expectations.length, violations.size());
		List<ConstraintViolation<BoardForm>> list = TestUtils.convertToListSortedByMessage(
				violations);
		for (int i = 0; i < list.size(); i++) {
			ConstraintViolation<BoardForm> violation = list.get(i);
